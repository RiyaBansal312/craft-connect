...
// ARTISAN APPLICATION
// ========================
router.post('/artisan-application', auth, async (req, res) => {
  try {
    const { location, serviceCategory, verificationType } = req.body;

    if (!location || !serviceCategory) {
      return res.status(400).json({ msg: 'Location and service category are required' });
    }
    ...
    // Update user to artisan role
    user.role = 'artisan';
    user.artisanInfo = {
      location,
      serviceCategory,
      verificationType: verificationType || 'MOBILE_OTP',
      status: 'approved', // Auto-approve since mobile/email verified
      rating: 0,
      totalRatings: 0,
      bio: '',
      skills: [],
    ...
    console.log(`✅ Artisan application approved for ${user.email}`);

    res.json({
      success: true,
      msg: 'Artisan application submitted successfully!',
      user: {
        id: user._id,
        name: user.name,
        email: user.email,
