...
import User from '../../models/User.js'; // ✅ Add at top
...
const isValidIndianMobile = (phone) => {
  ...
  // If starts with 91, check last 10 digits
  if (cleaned.startsWith('91') && cleaned.length === 12) {
    const number = cleaned.slice(2);
    return /^[6-9]\d{9}$/.test(number);
  }
  
  // Otherwise check if it's 10 digits starting with 6-9
